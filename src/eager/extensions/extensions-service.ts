...
import { Extension as ExtensionApi, ExtensionRegistrar as ExtensionRegistrarApi, Frame as FrameApi } from './api';
...
export class ExtensionsService implements FrameExtensionsAccessService {
    ...
    private _bundled: readonly BundledExtension[];
    ...
    setBundled(value: readonly BundledExtension[]) {
        this._bundled = value;
    }
        ...
        if (publisherType === undefined) {
            ...
            if (publisherTypeId === undefined) {
                ...
                if (activeDownload === undefined) {
                    // must have timed out
                    window.motifLogger.logWarning('Extension active download not found:',
                    ...
                    if (matchingExtension !== undefined) {
                        ErrorCodeLogger.logExternalError(ErrorCode.ExtensionsService_AddDuplicateName, this.generateExtensionKeyText(requestExtensionInfo));
                    } else {
...
export namespace ExtensionsService {
    ...
    export const nullEventHandler = () => {/* no-op */};
