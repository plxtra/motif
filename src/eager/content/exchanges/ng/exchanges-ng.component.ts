...
@Component({
    ...
    imports: [IntegerCaptionedRadioNgComponent, SvgButtonNgComponent, ExchangesGridNgComponent]
})
export class ExchangesNgComponent extends ContentComponentBaseNgDirective implements OnDestroy, AfterViewInit {
    ...
    private _gridComponent: ExchangesGridNgComponent;
            ...
            (listId) => (
                {
                    ...
                    title: MarketsService.List.idToDescription(listId, Strings[StringId.Exchanges]),
                }
            )
        );
    ...
    private openList(listId: MarketsService.ListId) {
        switch (listId) {
            case MarketsService.ListId.Known:
                this._gridComponent.setList(this._marketsService.exchanges);
                break;
            case MarketsService.ListId.EnvironmentDefault:
                this._gridComponent.setList(this._marketsService.defaultExchangeEnvironmentExchanges);
                break;
            case MarketsService.ListId.Unknown:
                this._gridComponent.setList(this._marketsService.unknownExchanges);
                break;
            default:
...
export namespace ExchangesNgComponent {
    ...
    export function create(container: ViewContainerRef, opener: LockOpenListItem.Opener) {
        ...
        const componentRef = container.createComponent(ExchangesNgComponent, { injector });
        assert(componentRef.instance instanceof ExchangesNgComponent, 'ENGC31175');
