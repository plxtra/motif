...
export class ResultOrderRequestStepFrame extends OrderRequestStepFrame {
    ...
    send(orderPad: OrderPad, definition: OrderRequestDataDefinition) {
        if (orderPad.sent) {
            const tradingIvemId = orderPad.tradingIvemId;
            const name = tradingIvemId === undefined ? '' : tradingIvemId.name;
    ...
    private processIncubatedDataItem(dataItem: OrderRequestDataItem) {
        if (dataItem.requestTypeId !== this._requestTypeId) {
            ...
            if (this._order !== undefined) {
                const orderMapKey = Order.generateMapKey(this._order.id, this._order.accountZenithCode)
                this.orderBecameAvailableEvent(orderMapKey);
            }
        }
    }
...
export namespace ResultOrderRequestStepFrame {
    export type OrderBecameAvailableEvent = (this: void, orderMapKey: string) => void;
