import { AssertInternalError, MultiEvent, Result } from '@pbkware/js-utils';
import { SaveManagement, StringId, Strings, UserAlertService } from '@plxtra/motif-core';
...
export class HideUnloadSaveService {
    ...
    private handleGlobalBeforeUnload(event: BeforeUnloadEvent) {
        ...
        if (needSavingCount > 0) {
            ...
            const alert = this._userAlertService.queueAlert(UserAlertService.Alert.Type.Id.DataSavingBeforeLeave, Strings[StringId.UserAlert_SavingChanges]);
            ...
            Promise.all(promises).then(
                (results) => {
                    ...
                    if (alert === undefined) {
                        ...
                        this._canLeaveAlert = this._userAlertService.queueAlert(UserAlertService.Alert.Type.Id.CanLeave, Strings[StringId.UserAlert_ChangesSaved]);
                    }
                },
                (reason: unknown) => {
