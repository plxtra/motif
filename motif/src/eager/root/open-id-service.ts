...
export class OpenIdService {
    ...
    private _openIdSignoutUri: string;
    ...
    startAuthentication(): void {
        ...
        const promise = this._userManager.signinRedirect();
        promise.then(
            () => {
                // Do nothing. The redirect will be handled by the browser.
            },
            (error: unknown) => {
                const errorMsg = getErrorMessage(error);
                const loginFailedMsg = `Login failed. Check identity provider/server and settings [${errorMsg}]`;
                window.motifLogger.logError(loginFailedMsg);
                this._userAlertService.queueAlert(UserAlertService.Alert.Type.Id.SigninCouldNotBeProcessed, errorMsg);
            }
        );
    }
    ...
    signOut() {
        const promise = this._userManager.signoutRedirect();
        AssertInternalError.throwErrorIfPromiseRejected(promise, 'OISSA44913');
    }
...
//            post_logout_redirect_uri: this._openIdSignoutUri,
        ...
        if (error !== undefined && error instanceof ErrorResponse) {
            ...
            switch (error.error) {
                ...
                default: {
                    ...
                    this.logErrorEvent(`Unknown SessionRenewalProblem: ${errorText}`);
                }
            }
        }
    ...
    private scheduleSilentRenewRestartAttempt(errorMessage: string) {
        if (this._silentRenewRestartAttemptDelayTimeoutHandle === undefined) {
            ...
            this._silentRenewRestartAttemptDelayTimeoutHandle = setTimeout(
                () => {
                    const promise = this.attemptRestartSilentRenew(errorMessage);
                    promise.then(
                        () => {/**/},
                        () => {window.motifLogger.logInfo('Restart silent renew failed')},
                    );
                },
