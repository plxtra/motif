...
import { RevServerNotificationId } from 'revgrid';
...
export class DepthFrame extends ContentFrame {
        ...
        if (element === undefined) {
            ...
            if (asStrResult.isErr()) {
                ...
                if (commaTextResult.isErr()) {
                    this._filterActive = false;
                    this._filterXrefs = DepthFrame.JsonDefault.filterXrefs;
                    window.motifLogger.logWarning(`DepthDataItem LoadLayoutConfig: Invalid FilterXrefs: (${CommaTextErr.errorIdPlusExtraToCodePlusExtra(commaTextResult.error)})`);
                } else {
    ...
    open(dataIvemId: DataIvemId, preferredDepthStyleId: DepthStyleId) {
        this.close();
        this._dataIvemId = dataIvemId;
        this._preferredDepthStyleId = preferredDepthStyleId;
        this.subscribeSecurityDataItem(dataIvemId);
    }
    ...
    private subscribeSecurityDataItem(dataIvemId: DataIvemId) {
    ...
    private processUsableSecurityDataItem() {
        ...
        switch (resolvedDepthStyleId) {
            case DepthStyleId.Full: {
                ...
                depthDefinition.code = this._dataIvemId.code;
                depthDefinition.marketZenithCode = this._dataIvemId.marketZenithCode;
            ...
            case DepthStyleId.Short: {
                ...
                levelDefinition.code = this._dataIvemId.code;
                levelDefinition.marketZenithCode = this._dataIvemId.marketZenithCode;
    ...
    private resolveDepthStyleId() {
        ...
        if (subscriptionDataTypeIds === undefined) {
            window.motifLogger.logWarning(`Security ${this._dataIvemId.name} does not have Subscription Data`);
                    ...
                    if (subscriptionDataTypeIds.includes(PublisherSubscriptionDataTypeId.DepthFull)) {
                        ...
                        if (subscriptionDataTypeIds.includes(PublisherSubscriptionDataTypeId.Depth)) {
                            ...
                            if (subscriptionDataTypeIds.includes(PublisherSubscriptionDataTypeId.DepthShort)) {
                                ...
                                window.motifLogger.logWarning(`Symbol ${this._dataIvemId.name} does not have any Depth`);
