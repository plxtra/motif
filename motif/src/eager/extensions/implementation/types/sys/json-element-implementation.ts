import { DecimalFactory, JsonElement } from '@pbkware/js-utils';
import {
...
export class JsonElementImplementation implements JsonElementApi {
    constructor(private readonly _actual: JsonElement, private readonly _decimalFactory: DecimalFactory) { }
    ...
    tryGetElement(name: string): JsonElementResultApi<JsonElementApi> {
        ...
        if (result.isErr()) {
            ...
            const jsonElement = new JsonElementImplementation(result.value, this._decimalFactory);
    ...
    tryGetElementArray(name: string): JsonElementResultApi<JsonElementApi[]> {
        ...
        if (result.isErr()) {
            ...
            const value = JsonElementImplementation.arrayToApi(result.value, this._decimalFactory);
    ...
    tryGetDecimal(name: string): JsonElementResultApi<DecimalApi> {
        const result = this._actual.tryGetDecimal(name, this._decimalFactory);
        if (result.isErr()) {
            return new JsonElementErrImplementation(result.error);
        } else {
    ...
    getDecimal(name: string, defaultValue: DecimalApi): DecimalApi {
        return this._actual.getDecimal(name, defaultValue, this._decimalFactory);
    }
    ...
    forEachElement(callback: JsonElementApi.ForEachElementCallback): void {
        this._actual.forEachElement((name: string, value: JsonElement, idx: IntegerApi) => {
            const valueApi = JsonElementImplementation.toApi(value, this._decimalFactory);
...
export namespace JsonElementImplementation {
    export function toApi(value: JsonElement, decimalFactory: DecimalFactory): JsonElementApi {
        return new JsonElementImplementation(value, decimalFactory);
    }
    ...
    export function arrayToApi(value: readonly JsonElement[], decimalFactory: DecimalFactory): JsonElementApi[] {
        const count = value.length;
        const result = new Array<JsonElementApi>(count);
        for (let i = 0; i < count; i++) {
            result[i] = toApi(value[i], decimalFactory);
        }
        return result;
    }
