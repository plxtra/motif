...
@Directive()
export abstract class ListItemSelectItemUiActionNgDirectiveNgDirective<T> extends SelectItemUiActionNgDirective<T> {
    public itemCaption = '';
    public itemTitle = '';
    ...
    initialiseEnum(action: SelectItemUiAction<T>, item: T) {
        this._item = item;
    ...
    protected override applyItemTitle(item: T, title: string) {
        ...
        if (item === this._item && title !== this.itemTitle) {
    ...
    protected override applyItemCaption(item: T, caption: string) {
        ...
        if (item === this._item && caption !== this.itemCaption) {
